...
# Joshua Reno & Giuseppe Pantalone
...
markersMain = {}
def markers(): return markersMain
...
async def marker_processing(robot, camera_settings, show_diagnostic_image=False):
    ...
    # Detect the markers
    markers, diag, label = detect.detect_markers(image, camera_settings, include_diagnostics=True)
    ...
    return marker_list, annotated_image, label
...
async def run(robot: cozmo.robot.Robot):
    ...
    while True:
        ...
        if localized:
            ...
            marker_list, image, labels = await marker_processing(robot, camera_settings)
            for i in range(len(marker_list)):
                if label[i] not in markersMain:
                    markersMain[label[i]] = marker_list[i]
